          ...
          {ugcImages.map((image) => (
            <CarouselItem key={image.id} className="pl-2 md:pl-4 basis-[45%] sm:basis-1/3 md:basis-1/4 lg:basis-1/5">
              <div className="relative group aspect-[3/4] overflow-hidden rounded-2xl bg-muted border border-border/40 shadow-sm hover:shadow-md transition-shadow duration-300">
                ...
                <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-r from-[#C7A867]/80 to-[#D5C3A5]/80 backdrop-blur-sm opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center px-3 py-3 md:px-4 md:py-4">
                  <p className="text-[#0C1520] text-xs md:text-sm font-medium text-center leading-snug">
                    {image.review}
                  </p>
                </div>
              </div>
